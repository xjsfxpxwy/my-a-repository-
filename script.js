...
// 注意：createPlaneModel 和 createSimplePlane 将在视频部分后定义，但已提前实现
...
function initBooleanPanelEvents() {
    ...
    document.getElementById('bool-tol-slider')?.addEventListener('input', function(e) {
        ...
        const val = document.getElementById('bool-tol-val');
        if (val) val.textContent = v.toExponential(0);
    });
    ...
    document.getElementById('bool-add-to-stack-btn')?.addEventListener('click', () => {
        ...
        const status = document.getElementById('bool-status-text');
        if (status) status.textContent = '已添加到修改器堆栈';
    });
}
...
function buildCameraPanel() {
    ...
    mp?.addEventListener('click', () => { mp.classList.add('active'); mo?.classList.remove('active'); if(pc) pc.style.display = 'block'; if(oc) oc.style.display = 'none'; window.applyProjection(); });
    mo?.addEventListener('click', () => { mo.classList.add('active'); mp?.classList.remove('active'); if(pc) pc.style.display = 'none'; if(oc) oc.style.display = 'block'; window.applyProjection(); });
    ...
    fs?.addEventListener('input', () => { if(fi) fi.value = fs.value; window.applyProjection(); });
    fi?.addEventListener('change', () => { let v = parseFloat(fi.value); v = Math.min(179, Math.max(1, v)); if(fs) fs.value = v; window.applyProjection(); });
    oss?.addEventListener('input', () => { if(osi) osi.value = oss.value; window.applyProjection(); });
    osi?.addEventListener('change', () => { let v = parseFloat(osi.value); v = Math.min(100, Math.max(0.1, v)); if(oss) oss.value = v; window.applyProjection(); });
    ...
    shapePanels.forEach(shapeId => {
        ...
        if (shapeKey === 'sphere') {
            ['rx','ry','rz'].forEach(axis => {
                ...
                if (s) s.addEventListener('change', pushHistory);
            });
        }
    });
...
function buildMaterialPanel() {
    ...
    document.getElementById('mat-color')?.addEventListener('input',function(){ const val = document.getElementById('mat-color-val'); if(val) val.textContent=this.value; applyMatFromUI(); });
    document.getElementById('mat-metalness')?.addEventListener('input',function(){ const val = document.getElementById('mat-metalness-val'); if(val) val.textContent=parseFloat(this.value).toFixed(2); applyMatFromUI(); });
    document.getElementById('mat-roughness')?.addEventListener('input',function(){ const val = document.getElementById('mat-roughness-val'); if(val) val.textContent=parseFloat(this.value).toFixed(2); applyMatFromUI(); });
    document.getElementById('mat-emissive')?.addEventListener('input',function(){ const val = document.getElementById('mat-emissive-val'); if(val) val.textContent=this.value; applyMatFromUI(); });
    document.getElementById('mat-emissive-intensity')?.addEventListener('input',function(){ const val = document.getElementById('mat-emissive-intensity-val'); if(val) val.textContent=parseFloat(this.value).toFixed(2); applyMatFromUI(); });
    document.getElementById('mat-transparent')?.addEventListener('change',applyMatFromUI);
    document.getElementById('mat-opacity')?.addEventListener('input',function(){ const val = document.getElementById('mat-opacity-val'); if(val) val.textContent=parseFloat(this.value).toFixed(2); applyMatFromUI(); });
    ...
    function loadMatTex(fid,key){ const inp=document.getElementById(fid); if(!inp) return; inp.addEventListener('change',()=>{ const file=inp.files[0]; if(!file)return; createMediaTexture(file,tex=>{ tex.repeat.set(parseFloat(document.getElementById('tex-repeat-u')?.value)||1,parseFloat(document.getElementById('tex-repeat-v')?.value)||1); tex.offset.set(parseFloat(document.getElementById('tex-offset-u')?.value)||0,parseFloat(document.getElementById('tex-offset-v')?.value)||0); tex.rotation=parseFloat(document.getElementById('tex-rotation')?.value)||0; tex.needsUpdate=true; if(materialTextures[key]){ if(materialTextures[key].isVideoTexture){ const v=materialTextures[key].image||materialTextures[key].source; if(v&&v.pause){ v.pause(); v.removeAttribute('src'); v.load(); } } materialTextures[key].dispose(); } materialTextures[key]=tex; applyMatFromUI(); inp.value=''; }); }); }
    ...
    document.querySelectorAll('.preset-btn[data-mat-preset]').forEach(b=>{ b.addEventListener('click',()=>{ const presets={metal:{color:'#c0c0c0',metalness:1.0,roughness:0.2,emissive:'#000000',emissiveIntensity:0,transparency:0.18},plastic:{color:'#e8e8e8',metalness:0.0,roughness:0.3,emissive:'#000000',emissiveIntensity:0,transparency:0.18},rubber:{color:'#333333',metalness:0.0,roughness:0.9,emissive:'#000000',emissiveIntensity:0,transparency:0.18},glass:{color:'#ddeeff',metalness:0.1,roughness:0.05,emissive:'#000000',emissiveIntensity:0,transparency:0.5},emissive:{color:'#ff6600',metalness:0.5,roughness:0.3,emissive:'#ff4400',emissiveIntensity:1.5,transparency:0.18}}; const p=presets[b.dataset.matPreset]; if(!p)return; document.getElementById('mat-color').value=p.color; const mcv = document.getElementById('mat-color-val'); if(mcv) mcv.textContent=p.color; document.getElementById('mat-metalness').value=p.metalness; const mmv = document.getElementById('mat-metalness-val'); if(mmv) mmv.textContent=p.metalness.toFixed(2); document.getElementById('mat-roughness').value=p.roughness; const mrv = document.getElementById('mat-roughness-val'); if(mrv) mrv.textContent=p.roughness.toFixed(2); document.getElementById('mat-emissive').value=p.emissive; const mev = document.getElementById('mat-emissive-val'); if(mev) mev.textContent=p.emissive; document.getElementById('mat-emissive-intensity').value=p.emissiveIntensity; const meiv = document.getElementById('mat-emissive-intensity-val'); if(meiv) meiv.textContent=p.emissiveIntensity.toFixed(2); document.getElementById('mat-opacity').value=p.transparency; const mov = document.getElementById('mat-opacity-val'); if(mov) mov.textContent=p.transparency.toFixed(2); document.querySelectorAll('.preset-btn[data-mat-preset]').forEach(x=>x.classList.remove('active')); b.classList.add('active'); const standardRadio = document.querySelector('input[name="mat-type"][value="standard"]'); if(standardRadio) standardRadio.checked=true; applyMatFromUI(); }); });
    refreshMaterialPanel();
}
function refreshMaterialPanel(){
    if(!currentModel||!currentModel.material)return;
    const mat=currentModel.material;
    if(mat.color&&document.getElementById('mat-color')){
        document.getElementById('mat-color').value='#'+mat.color.getHexString();
        const val = document.getElementById('mat-color-val'); if(val) val.textContent='#'+mat.color.getHexString();
    }
    if(mat.metalness!==undefined){ const s=document.getElementById('mat-metalness'),v=document.getElementById('mat-metalness-val'); if(s&&v){ s.value=mat.metalness; v.textContent=mat.metalness.toFixed(2); } }
    if(mat.roughness!==undefined){ const s=document.getElementById('mat-roughness'),v=document.getElementById('mat-roughness-val'); if(s&&v){ s.value=mat.roughness; v.textContent=mat.roughness.toFixed(2); } }
    if(mat.emissive&&document.getElementById('mat-emissive')){ document.getElementById('mat-emissive').value='#'+mat.emissive.getHexString(); const val = document.getElementById('mat-emissive-val'); if(val) val.textContent='#'+mat.emissive.getHexString(); }
    if(mat.emissiveIntensity!==undefined){ const s=document.getElementById('mat-emissive-intensity'),v=document.getElementById('mat-emissive-intensity-val'); if(s&&v){ s.value=mat.emissiveIntensity; v.textContent=mat.emissiveIntensity.toFixed(2); } }
    const tc=document.getElementById('mat-transparent'),os=document.getElementById('mat-opacity'),ov=document.getElementById('mat-opacity-val');
    if(tc&&os&&ov){ tc.checked=!!mat.transparent; const curTransparency = mat.transparent ? (1 - mat.opacity) : 0.18; os.value=curTransparency; ov.textContent=curTransparency.toFixed(2); }
    const type=mat.isMeshBasicMaterial?'basic':mat.isMeshPhysicalMaterial?'physical':'standard';
    const radio=document.querySelector(`input[name="mat-type"][value="${type}"]`); if(radio) radio.checked=true;
}

// ========== 渲染面板 ==========
function buildRenderPanel(){
    const cont=document.getElementById('render-controls-container');
    if(!cont) return;
    cont.innerHTML=`<div class="control-group"><div class="control-title">背景</div><div class="control-row"><span class="control-label">背景颜色</span><input type="color" id="render-bg-color" value="#050518"><span id="render-bg-color-val">#050518</span></div></div><div class="control-group"><div class="control-title">阴影</div><div class="control-row"><span class="control-label">阴影类型</span><div class="option-group"><label class="option-label"><input type="radio" name="shadow-type" value="basic"> 基本</label><label class="option-label"><input type="radio" name="shadow-type" value="pcf"> PCF</label><label class="option-label selected"><input type="radio" name="shadow-type" value="pcf" checked> PCF</label></div></div><div class="control-row"><span class="control-label">贴图尺寸</span><div class="option-group"><label class="option-label"><input type="radio" name="shadow-map-size" value="512"> 512</label><label class="option-label selected"><input type="radio" name="shadow-map-size" value="1024" checked> 1024</label><label class="option-label"><input type="radio" name="shadow-map-size" value="2048"> 2048</label></div></div><div class="control-row"><span class="control-label">偏移(Bias)</span><input type="range" id="shadow-bias" min="0" max="0.01" value="0.0005" step="0.0001"><span class="slider-value" id="shadow-bias-val">0.0005</span></div></div><div class="control-group"><div class="control-title">抗锯齿</div><div class="option-group"><label class="option-label selected"><input type="radio" name="aa-method" value="none" checked> 无</label><label class="option-label"><input type="radio" name="aa-method" value="fxaa"> FXAA</label></div></div><div class="control-group"><div class="control-title">色调映射</div><div class="option-group"><label class="option-label"><input type="radio" name="tone-mapping" value="none"> 无</label><label class="option-label selected"><input type="radio" name="tone-mapping" value="aces" checked> ACES</label><label class="option-label"><input type="radio" name="tone-mapping" value="reinhard"> Reinhard</label></div><div class="control-row" style="margin-top:8px;"><span class="control-label">曝光度</span><input type="range" id="exposure" min="0.2" max="2" value="1.0" step="0.1"><span class="slider-value" id="exposure-val">1.0</span></div></div><div class="control-group"><div class="control-title">预设</div><div class="preset-grid"><button class="preset-btn active" data-render-preset="performance">性能</button><button class="preset-btn" data-render-preset="balanced">平衡</button><button class="preset-btn" data-render-preset="quality">质量</button><button class="preset-btn" data-render-preset="cinematic">电影</button></div></div>`;
    document.getElementById('render-bg-color')?.addEventListener('input',function(){ scene.background=new THREE.Color(this.value); const val = document.getElementById('render-bg-color-val'); if(val) val.textContent=this.value; });
    document.querySelectorAll('input[name="shadow-type"]').forEach(r=>r.addEventListener('change',()=>{ const v=document.querySelector('input[name="shadow-type"]:checked')?.value; const m={basic:THREE.BasicShadowMap,pcf:THREE.PCFShadowMap}; renderer.shadowMap.type=m[v]||THREE.PCFShadowMap; }));
    document.querySelectorAll('input[name="shadow-map-size"]').forEach(r=>r.addEventListener('change',()=>{ const sz=parseInt(document.querySelector('input[name="shadow-map-size"]:checked')?.value); if(dirLight&&dirLight.shadow){ dirLight.shadow.mapSize.width=sz; dirLight.shadow.mapSize.height=sz; dirLight.shadow.map?.dispose(); dirLight.shadow.map=null; } }));
    document.getElementById('shadow-bias')?.addEventListener('input',function(){ const val = document.getElementById('shadow-bias-val'); if(val) val.textContent=this.value; if(dirLight&&dirLight.shadow) dirLight.shadow.bias=-parseFloat(this.value); });
    document.querySelectorAll('input[name="aa-method"]').forEach(r=>r.addEventListener('change',()=>{ const v=document.querySelector('input[name="aa-method"]:checked')?.value; fxaaEnabled=(v==='fxaa'); fxaaPass.enabled=fxaaEnabled; }));
    document.querySelectorAll('input[name="tone-mapping"]').forEach(r=>r.addEventListener('change',()=>{ const v=document.querySelector('input[name="tone-mapping"]:checked')?.value; const m={none:THREE.NoToneMapping,aces:THREE.ACESFilmicToneMapping,reinhard:THREE.ReinhardToneMapping}; renderer.toneMapping=m[v]||THREE.ACESFilmicToneMapping; }));
    document.getElementById('exposure')?.addEventListener('input',function(){ const val = document.getElementById('exposure-val'); if(val) val.textContent=parseFloat(this.value).toFixed(1); renderer.toneMappingExposure=parseFloat(this.value); });
    document.querySelectorAll('.preset-btn[data-render-preset]').forEach(b=>b.addEventListener('click',()=>{ const pr={performance:{shadowSize:512,shadowType:'basic',toneMapping:'none',exposure:1.0,aa:'none'},balanced:{shadowSize:1024,shadowType:'pcf',toneMapping:'aces',exposure:1.0,aa:'none'},quality:{shadowSize:2048,shadowType:'pcf',toneMapping:'aces',exposure:1.0,aa:'fxaa'},cinematic:{shadowSize:2048,shadowType:'pcf',toneMapping:'aces',exposure:1.3,aa:'fxaa'}}; const p=pr[b.dataset.renderPreset]; if(!p)return; const szR=document.querySelector(`input[name="shadow-map-size"][value="${p.shadowSize}"]`); if(szR){ szR.checked=true; szR.dispatchEvent(new Event('change')); } const tyR=document.querySelector(`input[name="shadow-type"][value="${p.shadowType}"]`); if(tyR){ tyR.checked=true; tyR.dispatchEvent(new Event('change')); } const exp = document.getElementById('exposure'); if(exp) { exp.value = p.exposure; const expVal = document.getElementById('exposure-val'); if(expVal) expVal.textContent = p.exposure.toFixed(1); } renderer.toneMappingExposure=p.exposure; const tmR=document.querySelector(`input[name="tone-mapping"][value="${p.toneMapping}"]`); if(tmR){ tmR.checked=true; tmR.dispatchEvent(new Event('change')); } const aaR=document.querySelector(`input[name="aa-method"][value="${p.aa}"]`); if(aaR){ aaR.checked=true; aaR.dispatchEvent(new Event('change')); } document.querySelectorAll('.preset-btn[data-render-preset]').forEach(x=>x.classList.remove('active')); b.classList.add('active'); }));
}

// ========== 场景面板 ==========
function buildScenePanel(){
    const cont=document.getElementById('scene-controls-container');
    if(!cont) return;
    cont.innerHTML=`<div class="control-group"><div class="control-title">网格</div><div class="control-row"><button class="toggle-btn" id="grid-show-btn">显示</button><button class="toggle-btn active" id="grid-hide-btn">隐藏</button></div></div><div class="control-group"><div class="control-title">地平面</div><div class="control-row"><button class="toggle-btn" id="ground-show-btn">显示</button><button class="toggle-btn active" id="ground-hide-btn">隐藏</button></div><div class="control-row" style="margin-top:6px;"><span class="control-label">颜色</span><input type="color" id="ground-color-scene" value="#888888"></div><div class="control-row"><span class="control-label">大小</span><input type="range" id="ground-size" min="1" max="30" value="10" step="0.5"><span class="slider-value" id="ground-size-val">10.0</span></div><div class="control-row"><span class="control-label">纹理</span><button class="texture-ctrl-btn" id="ground-upload">⬆️</button><button class="texture-ctrl-btn clear-btn" id="ground-clear">🗑️</button></div></div><div class="control-group"><div class="control-title">背景</div><div class="control-row"><button class="toggle-btn" id="bg-show-btn">显示</button><button class="toggle-btn active" id="bg-hide-btn">隐藏</button></div><div class="control-row" style="margin-top:6px;"><span class="control-label">颜色</span><input type="color" id="bg-color-scene" value="#050518"></div><div class="control-row"><span class="control-label">纹理</span><button class="texture-ctrl-btn" id="bg-upload">⬆️</button><button class="texture-ctrl-btn clear-btn" id="bg-clear">🗑️</button></div></div>`;
    document.getElementById('grid-show-btn')?.addEventListener('click',()=>{ gridHelper.visible=true; document.getElementById('grid-show-btn')?.classList.add('active'); document.getElementById('grid-hide-btn')?.classList.remove('active'); });
    document.getElementById('grid-hide-btn')?.addEventListener('click',()=>{ gridHelper.visible=false; document.getElementById('grid-hide-btn')?.classList.add('active'); document.getElementById('grid-show-btn')?.classList.remove('active'); });
    document.getElementById('ground-show-btn')?.addEventListener('click',()=>{ groundMesh.visible=true; document.getElementById('ground-show-btn')?.classList.add('active'); document.getElementById('ground-hide-btn')?.classList.remove('active'); });
    document.getElementById('ground-hide-btn')?.addEventListener('click',()=>{ groundMesh.visible=false; document.getElementById('ground-hide-btn')?.classList.add('active'); document.getElementById('ground-show-btn')?.classList.remove('active'); });
    document.getElementById('ground-color-scene')?.addEventListener('input',function(){ groundMesh.material.color.set(this.value); });
    document.getElementById('ground-size')?.addEventListener('input',function(){ const sz=parseFloat(this.value); const val = document.getElementById('ground-size-val'); if(val) val.textContent=sz.toFixed(1); const og=groundMesh.geometry; groundMesh.geometry=new THREE.CircleGeometry(sz,64); og.dispose(); });
    document.getElementById('ground-upload')?.addEventListener('click',()=>document.getElementById('hidden-ground-file-input')?.click());
    document.getElementById('ground-clear')?.addEventListener('click',()=>{ if(groundTexture){ groundTexture.dispose(); groundTexture=null; } groundMesh.material.map=null; groundMesh.material.needsUpdate=true; });
    ...
    document.getElementById('bg-show-btn')?.addEventListener('click',()=>{ document.getElementById('bg-show-btn')?.classList.add('active'); document.getElementById('bg-hide-btn')?.classList.remove('active'); applySceneBg(); });
    document.getElementById('bg-hide-btn')?.addEventListener('click',()=>{ document.getElementById('bg-hide-btn')?.classList.add('active'); document.getElementById('bg-show-btn')?.classList.remove('active'); scene.background=null; });
    document.getElementById('bg-color-scene')?.addEventListener('input',applySceneBg);
    function applySceneBg(){ if(!document.getElementById('bg-show-btn')?.classList.contains('active'))return; if(bgTexture) scene.background=bgTexture; else scene.background=new THREE.Color(document.getElementById('bg-color-scene')?.value); }
    document.getElementById('bg-upload')?.addEventListener('click',()=>document.getElementById('hidden-bg-file-input')?.click());
    document.getElementById('bg-clear')?.addEventListener('click',()=>{ if(bgTexture){ bgTexture.dispose(); bgTexture=null; } applySceneBg(); });
...
function resetPositioning(){ cameraOffset={x:0,y:0,z:10}; const s1=document.getElementById('cam-offset-x-slider'), i1=document.getElementById('cam-offset-x-input'); if(s1) s1.value=0; if(i1) i1.value=0; const s2=document.getElementById('cam-offset-y-slider'), i2=document.getElementById('cam-offset-y-input'); if(s2) s2.value=0; if(i2) i2.value=0; const s3=document.getElementById('cam-offset-z-slider'), i3=document.getElementById('cam-offset-z-input'); if(s3) s3.value=10; if(i3) i3.value=10; syncCameraFromOffset(); document.getElementById('mode-persp')?.click(); }
function resetMaterial(){ const standardRadio = document.querySelector('input[name="mat-type"][value="standard"]'); if(standardRadio) standardRadio.checked=true; const setVal = (id,val) => { const el = document.getElementById(id); if(el) el.value=val; }; setVal('mat-color','#ff6600'); const mcv = document.getElementById('mat-color-val'); if(mcv) mcv.textContent='#ff6600'; setVal('mat-metalness','0.78'); const mmv = document.getElementById('mat-metalness-val'); if(mmv) mmv.textContent='0.78'; setVal('mat-roughness','0.22'); const mrv = document.getElementById('mat-roughness-val'); if(mrv) mrv.textContent='0.22'; setVal('mat-emissive','#000000'); const mev = document.getElementById('mat-emissive-val'); if(mev) mev.textContent='#000000'; setVal('mat-emissive-intensity','0.1'); const meiv = document.getElementById('mat-emissive-intensity-val'); if(meiv) meiv.textContent='0.10'; const tc = document.getElementById('mat-transparent'); if(tc) tc.checked=false; setVal('mat-opacity','0.18'); const mov = document.getElementById('mat-opacity-val'); if(mov) mov.textContent='0.18'; setVal('tex-repeat-u','1'); setVal('tex-repeat-v','1'); setVal('tex-offset-u','0'); setVal('tex-offset-v','0'); setVal('tex-rotation','0'); for(const k of Object.keys(materialTextures)){ if(materialTextures[k]){ if(materialTextures[k].isVideoTexture){ const v=materialTextures[k].image||materialTextures[k].source; if(v&&v.pause){ v.pause(); v.removeAttribute('src'); v.load(); } } materialTextures[k].dispose(); materialTextures[k]=null; } } Object.values(textureStore).forEach(tex=>{ if(tex&&tex.isTexture){ tex.repeat.set(1,1); tex.offset.set(0,0); tex.rotation=0; } }); if(currentModel){ const params={color:new THREE.Color('#ff6600'),metalness:0.78,roughness:0.22,emissive:new THREE.Color('#000000'),emissiveIntensity:0.1,side:THREE.DoubleSide,transparent:false,opacity:1.0}; const oldMat=currentModel.material; const newMat=new THREE.MeshStandardMaterial(params); if(Array.isArray(currentModel.material)) currentModel.material=newMat; else if(currentModel.isGroup) currentModel.children.forEach(c=>{ if(c.material){ c.material.dispose(); c.material=newMat.clone(); } }); else currentModel.material=newMat; if(oldMat&&oldMat!==newMat) oldMat.dispose(); } document.querySelectorAll('.preset-btn[data-mat-preset]').forEach(b=>b.classList.remove('active')); }
function resetRender(){ scene.background=new THREE.Color('#050518'); const bgc = document.getElementById('render-bg-color'); if(bgc) bgc.value='#050518'; const bgcv = document.getElementById('render-bg-color-val'); if(bgcv) bgcv.textContent='#050518'; const st=document.querySelector('input[name="shadow-type"][value="pcf"]'); if(st){ st.checked=true; st.dispatchEvent(new Event('change')); } const ss=document.querySelector('input[name="shadow-map-size"][value="1024"]'); if(ss){ ss.checked=true; ss.dispatchEvent(new Event('change')); } const sb = document.getElementById('shadow-bias'); if(sb) sb.value=0.0005; const sbv = document.getElementById('shadow-bias-val'); if(sbv) sbv.textContent='0.0005'; dirLight.shadow.bias=-0.0005; const aaNone = document.querySelector('input[name="aa-method"][value="none"]'); if(aaNone) aaNone.checked=true; fxaaPass.enabled=false; fxaaEnabled=false; const tm=document.querySelector('input[name="tone-mapping"][value="aces"]'); if(tm){ tm.checked=true; tm.dispatchEvent(new Event('change')); } const exp = document.getElementById('exposure'); if(exp) exp.value=1.0; const expv = document.getElementById('exposure-val'); if(expv) expv.textContent='1.0'; renderer.toneMappingExposure=1.0; document.querySelectorAll('.preset-btn[data-render-preset]').forEach(b=>b.classList.remove('active')); const perf=document.querySelector('.preset-btn[data-render-preset="performance"]'); if(perf) perf.classList.add('active'); }
function resetScene(){ gridHelper.visible=true; document.getElementById('grid-show-btn')?.classList.add('active'); document.getElementById('grid-hide-btn')?.classList.remove('active'); groundMesh.visible=false; document.getElementById('ground-hide-btn')?.classList.add('active'); document.getElementById('ground-show-btn')?.classList.remove('active'); groundMesh.material.color.set('#888888'); const gcs = document.getElementById('ground-color-scene'); if(gcs) gcs.value='#888888'; const gs = document.getElementById('ground-size'); if(gs) gs.value=10; const gsv = document.getElementById('ground-size-val'); if(gsv) gsv.textContent='10.0'; const og=groundMesh.geometry; groundMesh.geometry=new THREE.CircleGeometry(10,64); og.dispose(); if(groundTexture){ groundTexture.dispose(); groundTexture=null; } groundMesh.material.map=null; groundMesh.material.needsUpdate=true; document.getElementById('bg-hide-btn')?.classList.add('active'); document.getElementById('bg-show-btn')?.classList.remove('active'); const bcs = document.getElementById('bg-color-scene'); if(bcs) bcs.value='#050518'; if(bgTexture){ bgTexture.dispose(); bgTexture=null; } scene.background=new THREE.Color('#050518'); }
...
// 工具按钮映射与UI初始化
document.querySelectorAll('.btn-left[data-shape]').forEach(btn=>{ if(btn.dataset.shape=='sphere') btn.classList.add('selected-left'); btn.addEventListener('click',()=>{ document.querySelectorAll('.btn-left[data-shape]').forEach(b=>b.classList.remove('selected-left')); btn.classList.add('selected-left'); switchToShape(btn.dataset.shape); }); });
...
document.getElementById('new-settings-btn')?.addEventListener('click',()=>{ const tpid=(currentShape==='frustum_cone'?'frustum':currentShape==='prism_frustum'?'prism-frustum':currentShape==='square_tube'?'square-tube':currentShape)+'-settings-panel'; const p=document.getElementById(tpid); if(p){ const wv=!p.classList.contains('visible'); p.classList.toggle('visible'); if(wv) ensurePanelInitialPosition(p); } });
...
function setActiveSubPanel(panel) {
    if (activeSubPanel === panel) {
        if (panel) panel.style.display = 'none';
        activeSubPanel = null;
    } else {
        if (activeSubPanel) activeSubPanel.style.display = 'none';
...
bottomBtns.forEach(btn => btn.addEventListener('click', e => {
    e.stopPropagation();
    const category = btn.dataset.category;
    if (btn.id === 'toggle-all-ui-btn') {
        globalUiHidden = !globalUiHidden;
        const hiddenSelector = '.top-shape-container, .sub-tools-container, .bottom-group button:not(#toggle-all-ui-btn)';
        document.querySelectorAll(hiddenSelector).forEach(el => el.classList.toggle('hidden-global', globalUiHidden));
    } else if (category === 'shape') {
        setActiveSubPanel(null);
        bottomBtns.forEach(b => b.classList.remove('selected-bottom'));
        btn.classList.add('selected-bottom');
    } else if (category === 'style') {
        setActiveSubPanel(stylePanel);
        bottomBtns.forEach(b => b.classList.remove('selected-bottom'));
        btn.classList.add('selected-bottom');
    } else if (category === 'combo') {
        setActiveSubPanel(buildPanel);
        bottomBtns.forEach(b => b.classList.remove('selected-bottom'));
        btn.classList.add('selected-bottom');
        ...
        if (scenePanel) {
            const isVisible = scenePanel.classList.contains('visible');
            closeAllPanels();
            if (!isVisible) {
...
// 构建工具面板内部按钮事件（确保点击切换面板，并延迟初始化）
const buildToolsPanelEl = document.getElementById('build-tools-panel');
if (buildToolsPanelEl) {
    const buildBtns = buildToolsPanelEl.querySelectorAll('.tool-btn[data-build-tool]');
...
document.querySelectorAll('.tool-btn[data-tool]').forEach(btn => btn.addEventListener('click', e => {
    ...
    if (tool === 'texture') {
        ...
        if (iv) {
            ...
            if (texturePanel) ensurePanelInitialPosition(texturePanel);
            updateTexturePanelContent();
        }
        return;
    }
...
// 顶部导航按钮
document.getElementById('nav-back-btn')?.addEventListener('click', undo);
document.getElementById('nav-forward-btn')?.addEventListener('click', redo);
document.getElementById('global-reset-btn')?.addEventListener('click', resetAll);
...
function pushHistory() {
    ...
    if (history.stack.length > history.maxSteps) {
        history.stack.shift();
    }
...
function applySnapshot(snapshot) {
    if (snapshot.shape !== currentShape) {
        switchToShape(snapshot.shape);
    }
    currentModel.position.copy(snapshot.position);
    currentModel.scale.copy(snapshot.scale);
    currentModel.rotation.copy(snapshot.rotation);
    controls.target.copy(currentModel.position);
    syncCameraFromOffset();
}
...
function undo() {
    if (history.currentIndex > 0) {
        ...
        applySnapshot(history.stack[history.currentIndex]);
    }
    updateNavButtons();
}
...
function updateNavButtons() {
    ...
    if (backBtn) backBtn.style.opacity = history.currentIndex > 0 ? '1' : '0.4';
    if (fwdBtn) fwdBtn.style.opacity = history.currentIndex < history.stack.length - 1 ? '1' : '0.4';
}
...
function resetAll() {
    ...
    if (currentModel && initPos) {
        ...
        if (currentShape === 'sphere') {
            document.getElementById('rx-slider').value = 1;
            document.getElementById('ry-slider').value = 1;
            document.getElementById('rz-slider').value = 1;
            document.getElementById('rx-input').value = 1;
            document.getElementById('ry-input').value = 1;
            document.getElementById('rz-input').value = 1;
        }
    }
    syncCameraFromOffset();
    if (currentShape === 'plane') syncPlaneRotation();
